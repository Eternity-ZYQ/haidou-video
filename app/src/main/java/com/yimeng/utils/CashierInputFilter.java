 ...
 * <pre>
 *  Author : huiGer
 *  Time   : 2019/5/23 11:42 AM.
 *  Email  : zhihuiemail@163.com
 *  Desc   : 限制输入只能输入金额类型
 *
 *  使用步骤： 1。限制只能输入数字 android:inputType="numberDecimal"
 *              或者setInputType(InputType.TYPE_NUMBER_FLAG_DECIMAL)
 *
 *           2。InputFilter[] filters = {new CashierInputFilter()};
...
public class CashierInputFilter implements InputFilter {
    ...
    public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
        ...
        // 开头就是点
        if(TextUtils.isEmpty(destText) && sourceText.equals(POINTER)) {
            return "";
        }
